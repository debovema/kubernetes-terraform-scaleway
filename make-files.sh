...
do
  case \$arg in
    'master')
      ...
      certbot certonly --standalone -d dashboard.\$DOMAIN_NAME -m \$LE_MAIL --agree-tos -n \$LE_STAGING
      mkdir -p /tmp/certs
      cp /etc/letsencrypt/live/dashboard.\$DOMAIN_NAME/fullchain.pem /tmp/certs/dashboard.crt
      cp /etc/letsencrypt/live/dashboard.\$DOMAIN_NAME/privkey.pem /tmp/certs/dashboard.key
